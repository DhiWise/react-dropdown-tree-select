...
class DropdownTreeSelect extends Component {
  static propTypes = {
    ...
    noData: PropTypes.bool, //For when data={} object to show No Data
  ...
  onCheckboxChange = (id, checked, callback) => {
    ...
    if (!tags.length) {
      //Give allow to remove default value
    ...
    this.props.onChange({ currentNode: node, selectedNode: tags })
  }
        ...
        >
          ...
          {showDropdown && (
            <div className="dropdown-content" {...this.getAriaAttributes()}>
              {inlineSearchInput && searchInput}
              {this.state.allNodesHidden || this.props.noData ? (
                <span className="no-matches">{texts.noMatches || 'No options found'}</span>
              ) : (
                <Tree
